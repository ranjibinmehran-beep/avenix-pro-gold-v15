from execution import get_tehran_time
...
TXT = {
    "fa": {
        ...
        "balance_title": "موجودی واقعی حساب بروکر (Balance)",
    ...
    "en": {
        ...
        "balance_title": "Live Broker Balance",
...
if not st.session_state.terms_accepted:
    ...
    with tab_broker_view:
        ...
        st.markdown("<div class='ios-card'>", unsafe_allow_html=True)
        current_b = config.get("broker_type", "forex_mt5").lower()
        b_idx = 1 if current_b == "crypto" else 0
        
        broker_opt = st.selectbox(
            t["broker_connect_title"],
            ["بروکر فارکس و پروپ‌فرم‌ها (MetaTrader 5)", "صرافی کریپتو (Binance, Bybit via CCXT)"],
            index=b_idx,
            key="broker_connect_sel"
        )
        selected_b = "crypto" if "صرافی" in broker_opt else "forex_mt5"
        ...
        st.markdown("### 💰 مدیریت و همگام‌سازی موجودی واقعی حساب بروکر")
        st.markdown("<div class='ios-card'>", unsafe_allow_html=True)
        current_balance = portfolio.get("balance", 10000.0)
        ...
        if st.button("🔄 ذخیره و اعمال آنی موجودی حساب واقعی بروکر", use_container_width=True):
            portfolio["balance"] = round(new_balance, 2)
            portfolio["initial_starting_balance"] = round(new_balance, 2)
            save_portfolio(portfolio)
            st.success("موجودی حساب واقعی با موفقیت همگام‌سازی شد و از این لحظه تمام سود و ضررهای زنده روی این موجودی اعمال می‌شوند!")
            time.sleep(1)
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    # ----------------- 🏆 TAB 7: ADVANCED RANKING & STATISTICS (رنکینگ و آمار پیشرفته) -----------------
    with tab_ranking_view:
        st.markdown("### 🏆 پنل رنکینگ و آمار پیشرفته عملکرد آونیکس (Avenix Performance & Win-Rate Analyzer)")
        st.markdown("<p style='color: #94a3b8; font-size: 13px;'>تحلیل دقیق معاملات بسته شده، سود/زیان روزانه و کلی، و نرخ برد زنده همگام‌سازی شده با ساعت گوشی شما (تهران)</p>", unsafe_allow_html=True)
        ...
        if len(completed_trades) > 0:
            ...
            # Ensure pnl and pnl_percent are float
            df_completed['pnl'] = df_completed['pnl'].astype(float)
            df_completed['pnl_percent'] = df_completed['pnl_percent'].astype(float)
            
            # --- CALCULATE STATISTICS ---
            total_trades = len(df_completed)
            winning_trades_df = df_completed[df_completed['pnl'] > 0]
            losing_trades_df = df_completed[df_completed['pnl'] <= 0]
            ...
            # Daily trades (using current Tehran date)
            today_tehran = get_tehran_time('%Y-%m-%d')
            
            # Extract date from close_time or open_time (format 'YYYY-MM-DD HH:MM:SS')
            df_completed['close_date'] = df_completed['close_time'].apply(lambda x: str(x).split(' ')[0])
            df_today = df_completed[df_completed['close_date'] == today_tehran]
            
            today_total_trades = len(df_today)
            today_winning_trades = len(df_today[df_today['pnl'] > 0])
            today_win_rate = (today_winning_trades / today_total_trades) * 100 if today_total_trades > 0 else 0.0
            ...
            with col_stat1:
                st.markdown(f"""
                <div style='text-align: center; padding: 10px; border-right: 1px solid #2e3e4f;'>
                    <span style='font-size: 28px;'>📊</span>
                    <div style='color: #94a3b8; font-size: 13px; margin-top: 5px;'>تعداد کل معاملات</div>
                    <div style='font-size: 22px; font-weight: 700; color: #f8fafc; margin-top: 5px;'>{total_trades}</div>
                </div>
                """, unsafe_allow_html=True)
            with col_stat2:
                st.markdown(f"""
                <div style='text-align: center; padding: 10px; border-right: 1px solid #2e3e4f;'>
                    <span style='font-size: 28px;'>🎯</span>
                    <div style='color: #94a3b8; font-size: 13px; margin-top: 5px;'>وین‌ریت کلی (Overall WR)</div>
                    <div style='font-size: 22px; font-weight: 700; color: #10b981; margin-top: 5px;'>{overall_win_rate:.1f}%</div>
                </div>
                """, unsafe_allow_html=True)
            with col_stat3:
                st.markdown(f"""
                <div style='text-align: center; padding: 10px; border-right: 1px solid #2e3e4f;'>
                    <span style='font-size: 28px;'>⚡</span>
                    <div style='color: #94a3b8; font-size: 13px; margin-top: 5px;'>وین‌ریت روزانه (Daily WR)</div>
                    <div style='font-size: 22px; font-weight: 700; color: #3b82f6; margin-top: 5px;'>{today_win_rate:.1f}%</div>
                </div>
                """, unsafe_allow_html=True)
            with col_stat4:
                ...
                <div style='text-align: center; padding: 10px;'>
                    <span style='font-size: 28px;'>💰</span>
                    <div style='color: #94a3b8; font-size: 13px; margin-top: 5px;'>کل سود و زیان (P&L)</div>
                    <div style='font-size: 22px; font-weight: 700; color: {pnl_color}; margin-top: 5px;'>{pnl_sign}${total_pnl:,.2f}</div>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            
            # --- DAILY SPECIFIC STATISTICS CARD ---
            st.markdown("<div class='ios-card'>", unsafe_allow_html=True)
            st.markdown("#### 📅 خلاصه وضعیت امروز شما")
            col_d1, col_d2, col_d3 = st.columns([1, 1, 1])
            with col_d1:
                st.metric("معاملات امروز", today_total_trades)
            with col_d2:
                st.metric("وین‌ریت امروز", f"{today_win_rate:.1f}%")
            with col_d3:
                st.metric("سود/زیان امروز", f"${today_pnl:,.2f}", delta=f"{today_pnl:,.2f}" if today_pnl != 0 else None)
            st.markdown("</div>", unsafe_allow_html=True)
            
            # --- HISTORY LIST OF DETAILED COMPLETED TRADES ---
            st.markdown("#### 📜 دفترچه جزئیات سود و زیان تک‌تک نمادها")
            
            # Render trade history table
            for idx, trade in df_completed.iloc[::-1].iterrows(): # Show most recent first
                side_label = "🟢 خرید (BUY)" if trade['side'] == "BUY" else "🔴 فروش (SELL)"
                pnl_val = float(trade['pnl'])
                pnl_color = "#10b981" if pnl_val >= 0 else "#ef4444"
                pnl_bg = "rgba(16, 185, 129, 0.1)" if pnl_val >= 0 else "rgba(239, 68, 68, 0.1)"
                pnl_sign = "+" if pnl_val >= 0 else ""
                
                st.markdown(f"""
                <div style='background-color: #1e293b; border: 1px solid #2e3e4f; border-right: 6px solid {pnl_color}; border-radius: 8px; padding: 12px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;'>
                    <div>
                        <strong style='font-size: 15px; color: #f8fafc;'>{trade['symbol']}</strong> 
                        <span style='font-size: 12px; color: #94a3b8; margin-right: 10px;'>({side_label})</span>
                        <div style='font-size: 12px; color: #64748b; margin-top: 4px;'>
                            📅 ورود: {trade['open_time']} | 📅 خروج: {trade['close_time']}
                        </div>
                    </div>
                    <div style='text-align: right;'>
                        <div style='background-color: {pnl_bg}; color: {pnl_color}; font-weight: 700; font-size: 15px; padding: 6px 12px; border-radius: 6px; display: inline-block;'>
                            {pnl_sign}${pnl_val:,.2f} ({pnl_sign}{trade['pnl_percent']:.2f}%)
                        </div>
                        <div style='font-size: 11px; color: #64748b; margin-top: 4px;'>
                            علت خروج: {trade.get('close_reason', 'N/A')}
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            st.markdown("<div class='ios-card' style='text-align: center; padding: 40px;'>", unsafe_allow_html=True)
