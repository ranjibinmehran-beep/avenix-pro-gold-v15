...
class TradingBrain:
    ...
    def analyze(self, symbol, multi_tf_data):
        """
        The Ultimate Living AI Trading Brain V8.5 (Upgraded with ADX Chop-Filter & ATR Dynamic Volatility):
        - Evaluates isolated indicator confirmations.
        - Integrates specialized 'Contest Mode' with larger targets and high-frequency triggers.
        - Uses ADX to identify if market is TRENDING or RANGING, adjusting indicator weights.
        - Employs ATR-based volatility sizing for dynamic risk-neutral SL and TP placements.
        """
        ...
        threshold = 60 if contest_mode else self.config.get("brain_score_threshold", 70)
        ...
        atr = last_row.get('ATR', current_price * 0.005)
        ...
        # --- 🕵️ STATE DETECTION: ADX Trend vs Range-Bound Filter ---
        is_trending_market = adx >= 23.0
        market_state_label = "STRONG TREND 🟢" if is_trending_market else "CHOP / RANGE-BOUND 🟡"
        ...
        # 1. EMA 200 Trend Check (Weighted high in Trending markets, ignored in Chops)
        if is_trending_market:
            if current_price > ema200:
                confirmations["EMA 200"] = "BULLISH (Trending Above) 🟢"
                buy_score += 20
            else:
                confirmations["EMA 200"] = "BEARISH (Trending Below) 🔴"
                sell_score += 20
        else:
            confirmations["EMA 200"] = "NEUTRAL (Chop ignored EMA200) 🟡"

        # 2. EMA 20/50 Crossover Check
        if is_trending_market:
            if ema20 > ema50:
                confirmations["EMA 20/50"] = "BULLISH (Golden Cross) 🟢"
                buy_score += 15
            else:
                confirmations["EMA 20/50"] = "BEARISH (Death Cross) 🔴"
                sell_score += 15
        else:
            # Range mode: we buy EMA crossovers in opposite direction (mean reversion) or ignore
            confirmations["EMA 20/50"] = "NEUTRAL (Chop ignored Crosses) 🟡"
        ...
        if is_trending_market:
            if current_price > max_cloud:
                confirmations["Ichimoku Cloud"] = "BULLISH (Above Cloud) 🟢"
                buy_score += 15
            elif current_price < min_cloud:
                confirmations["Ichimoku Cloud"] = "BEARISH (Below Cloud) 🔴"
                sell_score += 15
            else:
                confirmations["Ichimoku Cloud"] = "NEUTRAL (Inside Cloud) 🟡"
        else:
            # Range mode: Cloud acts as standard bounce level
            confirmations["Ichimoku Cloud"] = "NEUTRAL (Range-bound Cloud) 🟡"
        ...
        # 5. RSI (Heavily weighted and reversed in Chop markets for mean reversion!)
        if is_trending_market:
            if 50 < rsi < 70:
            ...
            # 🕵️ CRITICAL UPGRADE FOR RANGE-BOUND (CHOP) MARKETS:
            # RSI oversold/overbought are highly accurate buying/selling triggers here!
            if rsi <= 35:
                confirmations["RSI"] = f"OVERSOLD BUY SETUP ({round(rsi, 1)}) 🟢"
                buy_score += 40 # High weight for buying range lows
            elif rsi >= 65:
                confirmations["RSI"] = f"OVERBOUGHT SELL SETUP ({round(rsi, 1)}) 🔴"
                sell_score += 40 # High weight for selling range highs
            else:
                confirmations["RSI"] = f"NEUTRAL RANGE ({round(rsi, 1)}) 🟡"
        ...
        # 7. Bollinger Bands (Dynamic weighting based on state)
        ...
        if is_trending_market:
            ...
            # 🕵️ CRITICAL UPGRADE: Bollinger lower/upper band hits are highly profitable reversal points in ranges!
            if pct_b < 0.15:
                confirmations["Bollinger Bands"] = "LOWER BAND BOUNCE (Range Buy) 🟢"
                buy_score += 40
            elif pct_b > 0.85:
                confirmations["Bollinger Bands"] = "UPPER BAND PULLBACK (Range Sell) 🔴"
                sell_score += 40
            else:
                confirmations["Bollinger Bands"] = "NEUTRAL RANGE (No boundary hit) 🟡"

        # Include ADX Telemetry
        confirmations["Market State"] = f"{market_state_label} | ADX: {round(adx, 1)}"
        ...
        is_htf_aligned = True
        if not contest_mode and is_trending_market: # Trend filter only applied in trend-following modes
            if sensitivity == "low":
        ...
        state_suffix = f"\n🌐 **وضعیت ساختار بازار:** {market_state_label} (شاخص قدرت روند ADX: {round(adx, 1)})"
        
        if action == 'BUY':
            brochure_reason = (
                f"🦅 **گزارش بروشور تحلیلی و تاییدیه صعودی آونیکس (Avenix Upgraded Report)**{mode_suffix}\n\n"
                ...
                f" ├ 📈 **روند ساختاری:** بازار در وضعیت {'روندی' if is_trending_market else 'رنج و نوسانی'} است.\n"
                f" ├ ☁️ **ایچیموکو:** تاییده‌های تنکان و کیجون فعال گردیده است.\n"
                f" ├ 📊 **مومنتوم RSI:** شاخص قدرت در تراز مطلوب {round(rsi, 1)} است.\n"
                f" ├ ⚡ **سیستم MACD:** تقاطع طلایی مکدی صادر شده است.\n"
                f" └ 📉 **نوسان‌سنج باند بولینگر:** {'حمایت باند پایینی لمس شده است (ورود رنج)' if not is_trending_market else 'شتاب حرکتی تایید گردید.'}\n\n"
                f"🏆 **نتیجه‌گیری هوش مصنوعی:** با تایید این تریپل همپوشانی و عبور از حد نصاب امتیاز {threshold}٪، یک پوزیشن خرید (Long) با اهداف معاملاتی پیشرفته فعال گردید."
            )
        elif action == 'SELL':
            brochure_reason = (
                f"🦅 **گزارش بروشور تحلیلی و تاییدیه نزولی آونیکس (Avenix Upgraded Report)**{mode_suffix}\n\n"
                ...
                f" ├ 📉 **روند ساختاری:** بازار در وضعیت {'روندی' if is_trending_market else 'رنج و نوسانی'} است.\n"
                f" ├ ☁️ **ایچیموکو:** لایه‌های حمایتی ابر کومو شکست خورده‌اند.\n"
                f" ├ 📊 **مومنتوم RSI:** ریزش شاخص به تراز نزولی {round(rsi, 1)} غلبه مطلق فروشندگان را نشان می‌دهد.\n"
                f" ├ ⚡ **سیستم MACD:** تقاطع مرگ مکدی تایید گردید.\n"
                f" └ 📈 **نوسان‌سنج باند بولینگر:** {'مقاومت باند بالایی لمس شده است (ورود رنج)' if not is_trending_market else 'فشار ریزشی تایید گردید.'}\n\n"
                f"🏆 **نتیجه‌گیری هوش مصنوعی:** با همسویی کامل تایم‌فریم معاملاتی با روندهای بالا دست و عبور از حد نصاب امتیاز {threshold}٪، پوزیشن فروش (Short) صادر شد."
            )
        ...
        if action == 'BUY':
            if use_atr_targets:
                # 🕵️ ATR-BASED VOLATILITY SIZING FOR SL/TP
                sl = current_price - (atr * self.config.get("sl_ratio", 1.5))
                tp1 = current_price + (atr * tp1_ratio)
                tp2 = current_price + (atr * tp2_ratio)
                tp3 = current_price + (atr * tp3_ratio)
            else:
