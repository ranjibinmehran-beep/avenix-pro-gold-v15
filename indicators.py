...
def calculate_atr(df, period=14):
    """
    Calculates Average True Range (ATR) to measure volatility and set dynamic SL/TP.
    """
    if len(df) < period + 1:
        df['ATR'] = df['high'] - df['low']
        df['ATR'] = df['ATR'].replace(0, 1e-5)
        return df
    ...
    # Fill initial NaN values and handle zero
    df['ATR'] = df['ATR'].fillna(high_low).replace(0, 1e-5)
    return df

def calculate_adx(df, period=14):
    """
    Calculates Average Directional Index (ADX) to determine trend strength (Chop vs Trend).
    """
    if len(df) < period * 2:
        df['ADX'] = 20.0
        df['plus_di'] = 20.0
    ...
    # Smoothed curves using Wilder's Smoothing
